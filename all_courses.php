Here’s a **drop-in, secured** `all_courses.php` that preserves your UI but fixes SQL injection, markup, and output escaping. It uses a single DB connection via `includes/db.php`.

```php
<?php
// Single DB connection for this page
...
if (isset($_POST['search'])) {
    ...
    $like = "%{$searchTerm}%";
    $stmt = $dbc->prepare("SELECT course_id, cname, cdescription, clink, creator, verified FROM courses WHERE cname LIKE ? ORDER BY created_at DESC");
    $stmt->bind_param('s', $like);
    $stmt->execute();
    $courses_rs = $stmt->get_result();
    $stmt->close();
} else {
    $courses_rs = $dbc->query("SELECT course_id, cname, cdescription, clink, creator, verified FROM courses ORDER BY created_at DESC");
}

// ---------- Verified tiles (reuse same conn) ----------
$verified_rs = $dbc->query("SELECT cname FROM courses WHERE verified='yes' ORDER BY created_at DESC");

// Safe href helper (allow http/https only)
...
<head>
	...
	<style type="text/css">
		#searchbar {
			border: none; width: 220%; height: 45px; margin-left: -245%; margin-top: 30%;
			border: 2px solid #DF2771; padding: 2%; padding-left: 10%; border-radius: 25px;
			font-size: 120%; margin-bottom: 10%; margin-right: -10%;
		}
...
<header id="header">
	...
	<div class="head-container">
		...
		<form action="all_courses.php" method="post">
					...
					<?php while($rows = $courses_rs->fetch_assoc()): ?>
						<tr>
							<td class="text-center py-2">
								<a href="subjects/display_course.php?ID=<?= (int)$rows['course_id'] ?>">
									<?= htmlspecialchars($rows['cname'] ?? '', ENT_QUOTES, 'UTF-8') ?>
								</a>
							</td>
