...
if (isset($_POST['search'])) {
    ...
    $like = "%{$searchTerm}%";
    $stmt = $dbc->prepare("SELECT cname, cdescription FROM courses WHERE cname LIKE ? ORDER BY created_at DESC");
    $stmt->bind_param('s', $like);
    $stmt->execute();
    $courses_rs = $stmt->get_result();
    $stmt->close();
} else {
    $courses_rs = $dbc->query("SELECT cname, cdescription FROM courses ORDER BY created_at DESC");
}
...
<head>
	...
	<style type="text/css">
		#searchbar{border:none;width:220%;height:45px;margin-left:-245%;margin-top:30%;border:2px solid #DF2771;padding:2%;padding-left:10%;border-radius:25px;font-size:120%;margin-bottom:10%;margin-right:-10%}
...
<!-- Navigation Bar -->
	<header id="header">
		...
		<marquee style="background: hsla(255,70%,10%,0.5); margin-top: 50px;" direction="left" onmouseover="this.stop()" onmouseout="this.start()" scrollamount="20"><div class="marqu">Register or Login in Now to get FREE access to the courses!</div></marquee>

		<div class="head-container">
			<h2 id="title" class="text-center pt-4" style="font-weight:bold; color:black;">Courses</h2><br>

			<form action="see_courses.php" method="post">
				...
				<table class="content-table" style="margin-bottom:auto;">
					...
					<tbody>
						<?php if ($courses_rs && $courses_rs->num_rows): ?>
							...
							<?php endwhile; ?>
						<?php else: ?>
							<tr><td colspan="2" class="text-center py-2">No courses found.</td></tr>
						<?php endif; ?>
					</tbody>
				</table>
			</div>
